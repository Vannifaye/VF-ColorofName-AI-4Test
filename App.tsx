...
const App: React.FC = () => {
  ...
  const handleGenerate = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setHistory(prev => [newPersona, ...prev.slice(0, 9)]);
      setInputName('');
    } catch (err) {
    ...
    <div className="min-h-screen transition-all duration-1000 ease-in-out flex flex-col items-center p-4 md:p-8" style={backgroundStyle}>
              ...
              <div className="flex-1 text-center md:text-left">
                ...
                <div className="flex flex-wrap gap-3 mt-12 justify-center md:justify-start">
                  {persona.colors.map((color, i) => (
                    <div key={i} className="flex items-center gap-2 bg-black/20 rounded-full pl-1 pr-4 py-1 border border-white/10">
                      <div className="w-6 h-6 rounded-full border border-white/20" style={{ backgroundColor: color }}></div>
      ...
      {history.length > 0 && (
        <div className="w-full max-w-4xl mt-24 mb-16">
          <h3 className="text-2xl font-semibold mb-8">历史灵感</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {history.map((h) => (
              <button
                key={h.timestamp}
                onClick={() => loadFromHistory(h)}
                className="glass rounded-3xl p-6 text-left hover:scale-[1.02] transition-all"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="truncate">
                    <h4 className="font-bold truncate">{h.name}</h4>
                    <p className="text-xs text-white/60">{h.mood}</p>
                  </div>
                </div>
              </button>
            ))}
      ...
      <footer className="mt-auto py-12 text-white/40 text-sm">
        &copy; {new Date().getFullYear()} 色彩实验室 &middot; @Vannifaye
      </footer>
